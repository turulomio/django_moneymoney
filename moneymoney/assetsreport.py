...
def image_bytes_after_removing_white_space(bytes_):
    #Converts bytes to temp_file
    with NamedTemporaryFile(mode="w+b", suffix='.png', delete=False) as temp_file:
        temp_file.write(bytes_)
    #Trims white space
    p=run(f"convert -trim {temp_file.name} {temp_file.name}.trim.png",  shell=True)
    if p.returncode==0:
        #Alter bytes if converted
        with open(f"{temp_file.name}.trim.png", "r+b") as f:
            bytes_=f.read()
    else:
        print(_("There was an error triming image. Is convert (from Imagemagick) installed?"))
    return bytes_
...
def generate_assets_report(request, format):
    ...
    if vTotalLastYear.amount>=0:
        ...
        doc.addImageParagraph([image_bytes_after_removing_white_space(getGlobalBytes_from_base64(request,"base64_assetsreport_evolution.png")), ], 26, 14, "Illustration")
        doc.pageBreak()
    ...
    width=15
    height=13
    doc.addImageParagraph([image_bytes_after_removing_white_space(getGlobalBytes_from_base64(request, "base64_assetsreport_classes_by_percentage.png")), ], width, height-1, "Illustration")

    doc.addParagraph(_("Investments group by investment type"), "Heading 2")
    doc.addImageParagraph([image_bytes_after_removing_white_space(getGlobalBytes_from_base64(request, "base64_assetsreport_classes_by_producttype.png")), ], width, height, "Illustration")

    doc.addParagraph(_("Investments group by leverage"), "Heading 2")        
    doc.addImageParagraph([image_bytes_after_removing_white_space(getGlobalBytes_from_base64(request, "base64_assetsreport_classes_by_leverage.png")), ], width, height, "Illustration")

    doc.addParagraph(_("Investments group by investment product"), "Heading 2")
    doc.addImageParagraph([image_bytes_after_removing_white_space(getGlobalBytes_from_base64(request, "base64_assetsreport_classes_by_product.png")), ], width, height, "Illustration")

    doc.addParagraph(_("Investments group by Call/Put/Inline"), "Heading 2")
    doc.addImageParagraph([image_bytes_after_removing_white_space(getGlobalBytes_from_base64(request, "base64_assetsreport_classes_by_pci.png")), ], width, height, "Illustration")
