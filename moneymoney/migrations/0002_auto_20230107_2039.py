...
CREATE FUNCTION public.currency_factor(at_datetime timestamp with time zone, currency_from text, currency_to text) RETURNS numeric
    LANGUAGE plpgsql
    AS $$
DECLARE
    factor NUMERIC;
...
--
-- Name: investment_operations(integer, timestamp with time zone, text, text, text, integer); Type: FUNCTION; Schema: public; Owner: postgres
--
...
CREATE FUNCTION public.percentage(from_ numeric, to_ numeric, by_100 boolean DEFAULT true) RETURNS numeric
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.total_balance(p_at_datetime timestamp with time zone, user_currency text) RETURNS TABLE(accounts_user numeric, investments_user numeric, total_user numeric, investments_invested_user numeric)
    LANGUAGE plpython3u
    AS $_$
...
CREATE FUNCTION public.tt_investment_operations(p_investment_id integer, p_at_datetime timestamp with time zone, user_currency text) RETURNS text
    LANGUAGE plpython3u
    AS $_$
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('moneymoney', '0001_initial'),
    ]

    operations = [migrations.RunSQL(s)]
