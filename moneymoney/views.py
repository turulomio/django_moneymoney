...
class StrategiesViewSet(viewsets.ModelViewSet):
    ...
    def withbalance(self, request): 
        ...
        for strategy in qs:
            ...
            plio=investment_operations.PlInvestmentOperations.from_qs(timezone.now(), request.user.profile.currency, strategy.investments_queryset(), 1)
...
class InvestmentsClasses(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        plio=investment_operations.PlInvestmentOperations.from_qs(timezone.now(), request.user.profile.currency, qs_investments_active,  1)
...
class Alerts(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        plio_inactive=investment_operations.PlInvestmentOperations.from_qs(timezone.now(), request.user.profile.currency, qs,  2)
...
class InvestmentsViewSet(viewsets.ModelViewSet):
    ...
    def operations_evolution_chart(self, request, pk=None):
        ...
        plio=investment_operations.PlInvestmentOperations.from_ids(timezone.now(), request.user.profile.currency, [investment.id, ], 1)
        ...
        for i, dt in enumerate(datetimes_list):
            plio_dt=investment_operations.PlInvestmentOperations.from_ids( dt, request.user.profile.currency, [investment.id, ], 2)
...
def InvestmentsoperationsFull(request):
    ...
    plio=investment_operations.PlInvestmentOperations.from_ids( timezone.now(), request.user.profile.currency, ids, mode)
...
def ReportAnnualIncome(request, year):
    ...
    plio=investment_operations.PlInvestmentOperations.from_all( dt_year_to, request.user.profile.currency, 1)
...
def ReportAnnualIncomeDetails(request, year, month):
    ...
    def listdict_investmentsoperationshistorical(request, year, month, local_currency, local_zone):
        ...
        plio=investment_operations.PlInvestmentOperations.from_all( dt_year_month, request.user.profile.currency, 1)
...
def ReportEvolutionInvested(request, from_year):
    ...
    for year in range(from_year, date.today().year+1): 
        dt_from=dtaware_year_start(year, request.user.profile.zone)
        dt_to=dtaware_year_end(year, request.user.profile.zone)
        plio=investment_operations.PlInvestmentOperations.from_qs( dt_to, request.user.profile.currency, qs, 1)
...
def ReportsInvestmentsLastOperation(request):
    ...
    if method==0: #Separated investments
        plio=investment_operations.PlInvestmentOperations.from_qs( timezone.now(), request.user.profile.currency, investments, 1)
        ...
        plio=investment_operations.PlInvestmentOperations.from_merging_io_current( timezone.now(), request.user.profile.currency, investments, 1)
