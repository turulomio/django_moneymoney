-- Add simulation options to investment operations

DROP FUNCTION public.investment_operations;
DROP FUNCTION public.investment_operations_totals;


CREATE FUNCTION public.investment_operations(p_investment_id integer, p_at_datetime timestamp with time zone, user_currency text, tablename text DEFAULT 'investmentsoperations'::text, simulation_account_currency text DEFAULT NULL::text, simulation_products_id integer DEFAULT NULL::integer) RETURNS TABLE(io text, io_current text, io_historical text)
    LANGUAGE plpython3u
    AS $_$
## p_product_id se utiliza para pasar el producto merging functions
...
if simulation_products_id is None:
	...
	account_currency=data['accounts_currency']
else:
	data_plan=plpy.prepare(f'SELECT products.id as products_id, multiplier, products.currency as products_currency, productstypes_id from products, leverages where leverages.id=products.leverages_id and products.id=$1', ["integer"])
	data=plpy.execute(data_plan, (simulation_products_id,))[0]
	account_currency=simulation_account_currency
...
investment2account_at_datetime=plpy.execute(cf_plan, (p_at_datetime, data['products_currency'], account_currency))[0]['currency_factor']
...
# Dictionary2user datetime objects as keys
a2u={}
a2u[p_at_datetime]=plpy.execute(cf_plan, (p_at_datetime, account_currency, user_currency))[0]['currency_factor']

for row in plan.cursor( [p_investment_id, p_at_datetime]):
    a2u[row["datetime"]]=plpy.execute(cf_plan, (row['datetime'], account_currency, user_currency ))[0]['currency_factor']
...
for o in io:
    ...
    o['currency_account']=account_currency
...
for c in cur:
    ...
    c['currency_account']=account_currency
...
for h in hist:
    ...
    h['currency_account']=account_currency
...
ALTER FUNCTION public.investment_operations(p_investment_id integer, p_at_datetime timestamp with time zone, user_currency text, tablename text, simulation_account_currency text, simulation_products_id integer) OWNER TO postgres;

--
-- Name: investment_operations_totals(integer, timestamp with time zone, text); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.investment_operations_totals(p_investment_id integer, p_at_datetime timestamp with time zone, user_currency text) RETURNS TABLE(io text, io_current text, io_historical text)
    LANGUAGE plpython3u
    AS $_$
